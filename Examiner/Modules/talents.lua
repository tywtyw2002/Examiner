...
for i = 1, 3 do
    -- TabButtonTemplate
    local tab = CreateFrame("Button",ex:GetName().."Tab"..i,mod.page,"PlayerTalentTabTemplate");
    tab.id = i;
    tab:SetScript("OnClick",TalentsTab_OnClick);
    if (i == 1) then
        tab:SetPoint("TOPLEFT",45,36);
    else
        tab:SetPoint("LEFT",ex:GetName().."Tab"..(i - 1),"RIGHT");
    end
end
...
-- Init Talents
function mod:doInitTalents()
    if (self:IsShown()) then
        self:UpdateTalents();
    end
    -- Gather Talent Info
    activeSpec = cfg.shownSpec or GetActiveTalentGroup(isInspect) or 1;
    local maxTree = 1;
    local combined = 0;
    for i = 1, 3 do
        local _, _, pointsSpent = GetTalentTabInfo(i,isInspect,nil,activeSpec);
    ...
    -- Details
    local talentSpec = GetTalentTabInfo(maxTree,isInspect,nil,activeSpec);
    self.details:Clear();
    self.details:Add("Talents");
...
function mod:UpdateTalents()
    activeSpec = cfg.shownSpec or GetActiveTalentGroup(isInspect) or 1;
    local tabName, _, _, background = GetTalentTabInfo(activeTab,isInspect,nil,activeSpec);
    if (not tabName) then
    	return
    end
    --ex:SetBackgroundTexture(background);
    ...
    for i = 1, _G.GetNumTalentTabs() do
        ...
        local tabName, _, pointsSpent = _G.GetTalentTabInfo(i, isInspect, nil, activeSpec);
